    ...
    <div className="wf-shell">
      ...
      <div className="wf-shell-main">
        <div className="wf-appbar">
          <span className="bar-crumbs">Réseau / Chauffeurs</span>
                ...
                <thead>
                  <tr style={{ borderBottom: '1.5px dashed var(--ink)' }}>
                    {['Chauffeur', 'Statut', "Taux d'acceptation", 'Charge actuelle', 'Expérience'].map(h => (
                      <th key={h} className="mono" style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--ink-3)' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {drivers.map((d, i) => (
                    <tr key={d.id} style={{ borderBottom: i < drivers.length - 1 ? '1.2px dashed rgba(31,29,26,.12)' : 'none' }}>
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{
                            ...
                            background: 'var(--hi)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <span className="mono" style={{ fontSize: 12 }}>{d.currentLoad} mission{d.currentLoad !== 1 ? 's' : ''}</span>
                      </td>
                      <td style={{ padding: '10px 14px' }}>
                  ...
                  {drivers.length === 0 && (
                    <tr>
                      <td colSpan={5} className="mono muted" style={{ textAlign: 'center', padding: '32px 0', fontSize: 12 }}>
